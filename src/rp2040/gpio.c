 ...
 ****************************************************************/
    ...
    DECL_ENUMERATION_RANGE("pin", "gpio0", 0, GPIO_COUNT);
#endif    
...
{
    gpio_set_function(gpio, (gpio_function_t)func);
...
static void _gpio_set_irq_enabled(uint32_t gpio, uint32_t events, \
    bool enabled, io_bank0_irq_ctrl_hw_t *irq_ctrl_base) {
...
// override bits.
void gpio_set_function(uint32_t gpio, gpio_function_t fn) {
...
gpio_function_t gpio_get_function(uint32_t gpio) {
    ...
    if(unlikely(IS_EXPANDER(gpio)) ) {
        ...
        return (gpio_function_t) ((iobank0_hw->io[gpio].ctrl
            & IO_BANK0_GPIO0_CTRL_FUNCSEL_BITS)
