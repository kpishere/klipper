...
class ADS1X1X:
        ...
        if(cls.current_sensor is not None):  
            ...
            if(cls.current_operation == ADS1X1_OPERATIONS['SET_MUX']):
                ...
                if(cls.current_operation == ADS1X1_OPERATIONS['READ_CONVERSION']):
                    ...
                    try: 
                        sample = \
                            cls.read_register(cls.current_sensor.data.chip_addr,ADS1X1X_REG_POINTER['CONVERSION'], 2)
                    except Exception:
                    ...
                    cls.current_sensor.data._callback( \
                        cls.current_sensor.data.mcu.estimated_print_time(measured_time), \
    ...
    def handle_connect(self):
        ...
        if(ADS1X1X.sample_timer is None):
            ...
            ADS1X1X.sample_timer = ADS1X1X.reactor.register_timer(ADS1X1X._sample_ads1x1)
